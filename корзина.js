    ...
    value.split('').forEach(num => {
        if (await_close){
            ...
            if (count + 1 == value.length || await_close_count == 0) {
                // this_num += calk_plus_minus(await_close_value.replace(")", ""))
                console.log(calk_plus_minus(await_close_value.replace(")", "")), value, value.split('').slice(start_pos, count-start_pos+1).join(''), start_pos, count-start_pos+1);
                // if (oper == '+') result = start + Number(this_num)
                // if (oper == '-') result = start - Number(this_num)
                // if (oper == '*') {
                //     if (this_num == 0) this_num = 1
                //     result = start - last_elem + (last_elem * Number(this_num))
                // }
                // if (oper == '/') result = start - last_elem + (last_elem / Number(this_num))
                await_close = false
